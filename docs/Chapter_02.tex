...
近年来，众多研究致力于解决上述挑战。一些研究采用基于优化的方法，例如一项使用统一运动学映射进行实时优化的研究，这种方法速度快且平台通用性好，但由于数学模型简化和局部解的存在，
精度有所欠缺 \cite{qinAnyteleopGeneralVisionbased2023}。
一些研究人员转向基于学习的方法，
% 如图 \ref{fig:algorithm_framework}(b) 所示，
...
。然而，在上述方法中，仅使用单帧数据进行重定向，
在其操作过程中未提取连续输入帧之间的帧间相关性信息。因此，生成的机器人动作序列缺乏流畅性。
...
\section{时间信息的重要性}
上述算法和现存的大部分算法都只考虑了输入帧的静态特征，忽略了帧间依赖关系\cite{hengFramebyframeMotionRetargeting2024}。
针对带有噪声的原始动捕数据，基于 Transformer 的编码器-解码器结构可以充当“时空滤波器”，在保持运动意图的同时自动滤除高频抖动\cite{jiangMotiongptHumanMotion2023}\cite{rampasekRecipeGeneralPowerful2022}\cite{brohanRt1RoboticsTransformer2022}。这一特性正是 TransRetarget 能够解决机器人重定向结果不平滑问题的核心理论基础。
...
需要强调的是，虽然本文算法在功能上等同于全连接图神经网络，但它与具有固定连接关系的图卷积网络不同。Transformer 通过自注意力机制动态地调整节点间的关系权重，这使得模型在面对各种异构灵巧手结构和不同数据输入时具备更强的灵活性。此外，Transformer 天生适用于处理时序数据，并具有极高的可扩展性。
\section{本章小结}
